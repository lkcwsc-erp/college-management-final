...
const DocRequestForm = ({ myAdmission, onSubmitted }) => {
  ...
  <div style={{ marginBottom: 14, background: '#f8faff', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#555' }}>
    ...
    {docType==='BONAFIDE' && '📋 Workflow: You → Student Section → Admin → Principal → Approved → Issued (₹200)'}
    {docType==='ID_CARD' && '📋 Workflow: You → Accounts (fee) → Student Section (issue)'}
    {docType==='MARKSHEET' && '📋 Workflow: You → Exam Section (process & issue)'}
    {docType==='PROVISIONAL_DEGREE' && '📋 Workflow: You → Student Section → Admin → Principal → Approved → Issued (₹100)'}
    {docType==='DEGREE' && '📋 Workflow: You → Student Section → Admin → Principal → Approved → Issued (₹100)'}
    {docType==='MIGRATION' && '📋 Workflow: You → Student Section → Admin → Principal → Approved → Issued (₹200)'}
  </div>
)}
        ...
        {h}
          ...
          {activeTab === 'documents' && (
            <div>
              ...
              <p style={{ color: '#666', marginBottom: 12, fontSize: 14 }}>Apply for Transfer Certificate, Bonafide, ID Card, Marksheet and more.</p>

              {/* All year info banner */}
              <div style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', borderRadius: 10, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#2E7D32' }}>
                ✅ <strong>1st Year, 2nd Year, 3rd Year</strong> — sab students document request kar sakte hain.
                {myAdmission?.admissionYear && <span style={{ marginLeft: 8, background: '#1565C0', color: '#fff', padding: '2px 10px', borderRadius: 10, fontSize: 11, fontWeight: 700 }}>{myAdmission.admissionYear}</span>}
              </div>
