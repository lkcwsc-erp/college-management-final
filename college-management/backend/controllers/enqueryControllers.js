...
exports.submitEnquiry = async (req, res) => {
  try {
    const { studentFullName, gender, dateOfBirth, email, phone, notes } = req.body;

    if (!studentFullName || !gender || !email || !phone) {
      return res.status(400).json({
        success: false,
        message: 'Name, gender, email and phone are required'
      });
    }
