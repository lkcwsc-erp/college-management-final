...
const printBonafide = (adm) => {
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Times New Roman',serif;background:#fff;padding:10px}
    /* A4 half page = 148mm height approx */
    .page{max-width:700px;width:100%;margin:auto;border:1px solid #333;padding:18px 24px;min-height:380px;max-height:420px}
    .header{display:flex;align-items:center;gap:12px;border-bottom:2px double #1a237e;padding-bottom:10px;margin-bottom:10px}
    .header-text{flex:1;text-align:center}
    .trust{font-size:10px;color:#333}
    .college{font-size:15px;font-weight:bold;color:#1a237e;margin:2px 0;line-height:1.2}
    .affil{font-size:10px;color:#333}
    .contact{font-size:10px;color:#555}
    .title{text-align:center;font-size:14px;font-weight:bold;text-decoration:underline;letter-spacing:2px;margin:6px 0}
    .meta{display:flex;justify-content:space-between;font-size:11px;margin-bottom:10px}
    .body{font-size:12.5px;line-height:1.9;text-align:justify}
    .hl{font-weight:bold;border-bottom:1px solid #333}
    .sign-row{display:flex;justify-content:space-between;margin-top:24px;font-size:12px}
    .sign-box{text-align:center}
    .sign-line{border-top:1px solid #333;padding-top:4px;margin-top:20px;min-width:140px;font-weight:bold}
    .validity{font-size:10px;color:#555;margin-top:8px;font-style:italic}
    @media print{body{padding:0}.page{border:1px solid #333;max-height:none}}
  </style></head><body>
  <div class="page">
    <div class="header">
      ...
      <div class="header-text">
        ...
        <div class="college">Late Kalpana Chawala Arts &amp; Science Mahila Senior College, Gangakhed</div>
        <div class="affil">Affiliated to SNDT Women&#x2019;s University, Mumbai</div>
        <div class="contact">Lecturer Colony, Gangakhed, Dist. Parbhani - 431514 | +91 9307162914</div>
      </div>
    </div>
    <div class="title">BONAFIDE CERTIFICATE</div>
    <div class="meta">
      <span>Cert. No.: <strong>${certNo}</strong></span>
      <span>Date: <strong>${dateStr}</strong></span>
    </div>
    <div class="body">
      This is to certify that <span class="hl">${adm.applicantName||'_________________________'}</span>,
      <em>${dOrS}</em> <span class="hl">${adm.fatherName||'_________________________'}</span>,
      resident of <span class="hl">${adm.address||'_________________________'}</span>,
      is a <em>bona fide</em> student of this college for the academic year <span class="hl">${acadYear}</span>.
      She is currently enrolled in <span class="hl">${adm.courseType||'________'}</span>
      (Subject: <span class="hl">${adm.preferredSubject||'________'}</span>),
      <span class="hl">${adm.admissionYear||'________'}</span>.
      Her Student ID is <span class="hl">${adm.studentId||'________'}</span>,
      PRN is <span class="hl">${adm.prnNumber||'________'}</span>.
      Her conduct and character are <span class="hl">Good</span>.
      This certificate is issued on her request for the purpose of
      <span class="hl">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>.
    </div>
    <div class="sign-row">
      <div class="sign-box"><div class="sign-line">Office Superintendent</div></div>
      <div class="sign-box"><div class="sign-line">Principal</div></div>
    </div>
    <div class="validity">* Valid for 6 months from date of issue. For duplicate, apply with fees.</div>
  </div>
  <scri${'pt'}>window.onload=()=>{window.print()}</scri${'pt'}>
  </body></html>`;
  const w = window.open('','_blank','width=760,height=560'); w.document.write(html); w.document.close();
};
