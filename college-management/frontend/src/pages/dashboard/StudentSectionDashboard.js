...
const printIDCard = (adm) => {
  ...
  const dobStr = adm.dateOfBirth ? new Date(adm.dateOfBirth).toLocaleDateString('en-IN') : '—';
  const html = `<!DOCTYPE html><html><head><title>ID Card</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:Arial,sans-serif;background:#e8eaf6;padding:30px;display:flex;justify-content:center;gap:30px}
    .card{width:200px;border-radius:12px;overflow:hidden;box-shadow:0 6px 24px rgba(0,0,0,0.25);background:white}
    .card-top{background:linear-gradient(180deg,#0d47a1,#1565C0);padding:10px 8px;text-align:center}
    .logo{width:40px;height:40px;object-fit:contain;margin-bottom:3px}
    .college-name{color:#fff;font-size:7.5px;font-weight:700;line-height:1.3;margin-bottom:1px}
    .college-sub{color:rgba(255,255,255,0.8);font-size:6px}
    .id-badge{background:#ffd54f;color:#0d47a1;font-size:8px;font-weight:800;letter-spacing:1.5px;text-align:center;padding:3px;margin:0}
    .photo-area{padding:10px 0 6px;text-align:center;background:#f8faff}
    .photo-box{width:70px;height:80px;border:2px solid #0d47a1;border-radius:6px;margin:0 auto 4px;overflow:hidden;background:#e8eaf6;display:flex;align-items:center;justify-content:center;font-size:24px}
    .student-name{font-size:10px;font-weight:800;color:#0d47a1;text-align:center;padding:0 6px}
    .card-body{padding:6px 10px 0}
    .field{font-size:8.5px;color:#333;margin:3px 0;display:flex;gap:3px}
    .flabel{color:#555;min-width:46px;font-weight:600}
    .fval{color:#0d47a1;font-weight:700;word-break:break-all}
    .id-chip{background:#0d47a1;color:white;font-size:8px;font-weight:800;padding:3px 8px;border-radius:3px;display:block;text-align:center;margin:6px 8px 4px}
    .card-bottom{background:#0d47a1;padding:6px 8px}
    .sig-area{display:flex;justify-content:space-between;align-items:flex-end;padding:4px 8px 6px}
    .sig-line{border-top:1px solid #555;padding-top:2px;font-size:7px;color:#555;text-align:center;min-width:60px}
    .valid-row{display:flex;justify-content:space-between;font-size:7px}
    @media print{body{background:white;padding:0}.card{box-shadow:none}@page{size:A6;margin:5mm}}
  </style></head><body>
  <div class="card">
    <div class="card-top">
      <img src="${logo}" class="logo"/>
      <div class="college-name">Late Kalpana Chawla Women's<br/>Senior College, Gangakhed</div>
      <div class="college-sub">Affiliated to SNDT Women's University, Mumbai</div>
    </div>
    <div class="id-badge">STUDENT IDENTITY CARD</div>
    <div class="photo-area">
      <div class="photo-box">${adm.studentPhoto ? `<img src="${adm.studentPhoto}" style="width:100%;height:100%;object-fit:cover"/>` : '👩'}</div>
      <div class="student-name">${adm.applicantName||'—'}</div>
    </div>
    <div class="card-body">
      <div class="field"><span class="flabel">Course:</span><span class="fval">${adm.courseType||'—'}</span></div>
      <div class="field"><span class="flabel">Subject:</span><span class="fval">${adm.preferredSubject||'—'}</span></div>
      <div class="field"><span class="flabel">Year:</span><span class="fval">${adm.admissionYear||'—'}</span></div>
      <div class="field"><span class="flabel">DOB:</span><span class="fval">${dobStr}</span></div>
      <div class="field"><span class="flabel">Mobile:</span><span class="fval">${adm.phone||'—'}</span></div>
      <div class="field"><span class="flabel">Blood:</span><span class="fval">${adm.bloodGroup||'—'}</span></div>
    ...
    <div class="card-bottom">
      <div class="valid-row" style="color:rgba(255,255,255,0.85);padding:0 2px">
        <span>Valid: ${validYear}–${validYear+1}</span>
        <span>lkcwsc.vnssorg.com</span>
      </div>
      <div style="color:rgba(255,255,255,0.7);font-size:6px;text-align:center;margin-top:2px">Lecture Colony, Gangakhed, Dist. Parbhani – 431514</div>
    </div>
  </div>
  <scri${'pt'}>window.onload=()=>{window.print()}</scri${'pt'}></body></html>`;
  const w = window.open('','_blank','width=300,height=520'); w.document.write(html); w.document.close();
};
