...
const printBonafide = (adm) => {
  ...
  // Admission year → class year (1st/2nd/3rd)
  const admYr = adm.admissionYear ? parseInt(adm.admissionYear) : 0;
  const currYr = new Date().getFullYear();
  const classYearNum = admYr > 0 ? (currYr - admYr + 1) : 0;
  const classYear = classYearNum === 1 ? '1st Year'
    : classYearNum === 2 ? '2nd Year'
    : classYearNum === 3 ? '3rd Year'
    : adm.admissionYear ? adm.admissionYear : '';
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .page{background:white;width:720px;border:2.5px solid #000;box-shadow:0 4px 20px rgba(0,0,0,.15)}
    /* Header */
    ...
    /* Body */
    .body{padding:10px 16px 6px;font-size:13.5px;line-height:1.55;text-align:left}
    .body p{margin:0;padding:4px 0}
    ...
    <div class="body">
      <p>  This is to certify that Miss <span class="ul">&nbsp;${name}&nbsp;</span> 
    ...
    <div class="erp">
      <span>This is a system generated certificate. | ERP ID: <strong>ERP${certNo}</strong></span>
    </div>
...
const printIDCard = (adm) => {
  ...
  <div class="card">
    ...
    <div style="padding:6px 14px;background:white;display:flex;justify-content:space-between;align-items:flex-end;border-top:1px solid #e0e0e0">
      <div style="font-size:8px;color:#555">Valid: ${validYear}–${validYear+1}</div>
      <div style="text-align:center">
        <div style="border-top:1px solid #1a237e;padding-top:3px;font-size:8px;font-weight:bold;color:#1a237e;min-width:80px">Principal</div>
      </div>
    </div>
  ...
  <div style="text-align:center;margin-top:14px;display:flex;gap:10px;justify-content:center">
    <button onclick="window.print()" style="padding:7px 18px;background:#1a237e;color:white;border:none;border-radius:6px;font-size:13px;font-weight:bold;cursor:pointer">🖨️ Print</button>
...
const printBonafide = (adm, req) => {
  const logo = _COLLEGE_LOGO;
  const y = new Date().getFullYear();
  const m = new Date().getMonth() + 1;
  const acadYear = m >= 6 ? (y + '-' + String(y+1).slice(2)) : ((y-1) + '-' + String(y).slice(2));
  ...
  const courseFull = ct.includes('b.sc')||ct.includes('bsc')||ct.includes('science')
    ...
    : (adm?.courseType||'—');
  const name = adm?.applicantName || '—';
  const father = adm?.fatherName || adm?.guardianName || '—';
  const sid = adm?.studentId || '—';
  const yr = adm?.admissionYear || '—';
  const reasonPara = req?.reason
    ? '<p class="para">This certificate is issued at her/his request for the purpose of <span class="field">' + req.reason + '</span>.</p>'
    : '<p class="para">This certificate is issued at her/his request.</p>';

  const html = '<!DOCTYPE html><html><head><title>Bonafide Certificate</title>'
    + '<style>'
    + '*{margin:0;padding:0;box-sizing:border-box}'
    + 'body{font-family:\'Times New Roman\',Times,serif;background:#fff;padding:14px;font-size:13px}'
    + '.cert{width:190mm;border:2px solid #333;margin:0 auto}'
    + '.hdr{display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:2px solid #000}'
    + '.hlogo{width:58px;height:58px;object-fit:contain;flex-shrink:0}'
    + '.htxt{flex:1;text-align:center}'
    + '.htrust{font-size:9px;font-weight:700;color:#222}'
    + '.hname{font-size:15px;font-weight:900;color:#000;margin:3px 0;line-height:1.2}'
    + '.haddr{font-size:8.5px;color:#444;margin-top:2px}'
    + '.title{text-align:center;font-size:15px;font-weight:900;letter-spacing:2px;padding:8px 0;border-bottom:1px solid #000;text-decoration:underline;background:#f9f9f9}'
    + '.body{padding:18px 20px}'
    + '.ref{display:flex;justify-content:space-between;font-size:11px;margin-bottom:18px}'
    + '.field{font-weight:700;text-decoration:underline}'
    + '.para{font-size:13px;line-height:2;text-align:justify;margin-bottom:14px;color:#000}'
    + '.sigrow{display:flex;justify-content:space-between;padding-top:30px}'
    + '.sig{text-align:center;font-size:11px}'
    + '.sigline{border-top:1px solid #000;margin-top:24px;padding-top:3px;font-weight:700}'
    + '.footer{font-size:9px;color:#666;border-top:1px dashed #999;padding:6px 0 0;margin-top:14px;font-style:italic;text-align:center}'
    + '@media print{body{padding:0}.cert{width:100%}@page{size:A4 portrait;margin:8mm}}'
    + '</style></head><body>'
    + '<div class="cert">'
    + '<div class="hdr">'
    + '<img src="' + logo + '" class="hlogo"/>'
    ...
    + '<div class="hname">Late Kalpana Chawla Women\'s Senior College (LKCWSC)</div>'
    + '<div class="haddr">Affiliated to SNDT Women\'s University, Mumbai</div>'
    + '<div class="haddr">Gangakhed, Dist. Parbhani, Maharashtra &ndash; 431514 &nbsp;|&nbsp; +91 9307162914 &nbsp;|&nbsp; lkcwsc.vnssorg.com</div>'
    + '</div></div>'
    + '<div class="title">BONAFIDE CERTIFICATE</div>'
    + '<div class="body">'
    + '<div class="ref"><span>No.:&nbsp;<span class="field">&nbsp;' + sid + '&nbsp;</span></span><span>Date:&nbsp;<span class="field">&nbsp;' + dateStr + '&nbsp;</span></span></div>'
    + '<p class="para">This is to certify that <span class="field">' + name + '</span>, daughter/son of <span class="field">' + father + '</span>, bearing Student ID <span class="field">' + sid + '</span>, is a bonafide student of this college. She/He is currently studying in <span class="field">' + yr + '</span> of <span class="field">' + courseFull + '</span> during the academic year <span class="field">' + acadYear + '</span>.</p>'
    + reasonPara
    + '<p class="para">We wish her/him all the best for her/his future endeavours.</p>'
    + '<div class="sigrow">'
    + '<div class="sig"><div class="sigline">Class Teacher</div></div>'
    + '<div class="sig"><div class="sigline">Student Section<br/>LKCWSC</div></div>'
    + '<div class="sig"><div class="sigline">Principal<br/>LKCWSC</div></div>'
    + '</div>'
    + '<p class="footer">This is a system-generated certificate. Valid for academic year ' + acadYear + ' only.</p>'
    + '</div></div>'
    + '<scri' + 'pt>window.onload=()=>{window.print()}</scri' + 'pt></body></html>';
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                      {myRequests.filter(r => r.status === 'completed').map((r, i) => (
                        <div key={i} style={{ background: '#fff', borderRadius: 10, border: '1px solid #c8e6c9', padding: '10px 16px', fontSize: 13 }}>
                          ...
                          {r.documentType === 'BONAFIDE' && (
                            <button onClick={() => printBonafide(myAdmission, r)} style={{ marginTop: 6, background: '#1565C0', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 12px', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                              🖨️ Print Certificate
                            </button>
                          )}
                        </div>
                      ...
                      {myRequests.map((req, i) => {
                        ...
                        return (
                          <div key={i} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e0e7ef', padding: 16, borderLeft: `4px solid ${ss.color}`, boxShadow: '0 1px 6px rgba(0,0,0,.05)' }}>
                            ...
                            {req.documentType === 'BONAFIDE' && (req.status === 'pending_generation' || req.status === 'completed') && (
                              <button onClick={() => printBonafide(myAdmission, req)} style={{ marginTop: 8, background: '#1565C0', color: '#fff', border: 'none', borderRadius: 7, padding: '6px 16px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                                🖨️ Print Bonafide Certificate
                              </button>
                            )}
                            {/* Workflow progress */}
