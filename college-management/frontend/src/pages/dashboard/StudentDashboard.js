                ...
                <>
                  ...
                  <div className="dash-cards" style={{ marginBottom: 24 }}>
                    {(() => {
                      ...
                      const yearlyData = courseKey && myAdmission.admissionYear ? OFFICIAL_FEES_YEARLY[courseKey]?.years[myAdmission.admissionYear] : null;
                      const grossYearly = yearlyData ? yearlyData.total : (myAdmission.totalFees || 0);
                      const schol = myAdmission.scholarshipAmount || 0;
                      const netPayable = Math.max(0, grossYearly - schol);
                      ...
                      return (
                        <>
                          <div className="dash-card blue">
                            <div className="dash-card-icon">📋</div>
                            <div><h3>{grossYearly > 0 ? `₹${grossYearly.toLocaleString('en-IN')}` : '—'}</h3><p>Yearly Gross Fee</p></div>
                          </div>
                    ...
                    return grossYearly > 0 ? (
                      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e0e7ef', marginBottom: 20, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,.05)' }}>
                        <div style={{ background: '#1565C0', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <h4 style={{ color: '#fff', margin: 0, fontSize: 14 }}>💰 Fee Statement — {myAdmission.admissionYear} ({myAdmission.courseType})</h4>
                          {yearlyData && <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: 12 }}>Sem I: ₹{yearlyData.sem1.toLocaleString('en-IN')} + Sem II: ₹{yearlyData.sem2.toLocaleString('en-IN')}</span>}
                        </div>
                        <div style={{ padding: '14px 20px', fontSize: 14 }}>
                          {[
                            { label: '📋 Gross Yearly Fee', value: grossYearly, color: '#222' },
                            ...(schol > 0 ? [{ label: '🏅 − Scholarship Deduction', value: schol, color: '#6A1B9A', prefix: '−' }] : []),
                            { label: '= Net Payable', value: netPayable, color: '#1565C0', bold: true },
                            { label: '✅ − Amount Paid', value: paid, color: '#2E7D32', prefix: '−' },
                            { label: pending > 0 ? '⚠️ Balance Pending' : '✅ Fully Paid', value: pending, color: pending > 0 ? '#C62828' : '#2E7D32', bold: true, big: true },
                          ].map((row, i) => (
                            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < 4 ? '1px solid #f0f0f0' : 'none' }}>
                              <span style={{ color: row.color, fontWeight: row.bold ? 700 : 500, fontSize: row.big ? 15 : 14 }}>{row.label}</span>
                              <span style={{ color: row.color, fontWeight: row.bold ? 800 : 600, fontSize: row.big ? 18 : 14 }}>
                                {row.prefix === '−' ? '−' : ''}₹{row.value.toLocaleString('en-IN')}
                              </span>
                            </div>
                          ))}
                        </div>
                        {/* All years fee preview */}
                        {courseKey && (
                          <div style={{ borderTop: '1px solid #e0e7ef', padding: '12px 20px', background: '#f8faff' }}>
                            <p style={{ fontSize: 12, color: '#888', marginBottom: 8, fontWeight: 600 }}>📊 Complete Fee Schedule ({OFFICIAL_FEES_YEARLY[courseKey]?.label})</p>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                              {Object.entries(OFFICIAL_FEES_YEARLY[courseKey]?.years || {}).map(([yr, data]) => {
                                const isCurrentYear = yr === myAdmission.admissionYear;
                                return (
                                  <div key={yr} style={{ background: isCurrentYear ? '#e3f2fd' : '#fff', border: `1px solid ${isCurrentYear ? '#1565C0' : '#e0e7ef'}`, borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                                    <div style={{ fontSize: 12, fontWeight: 700, color: isCurrentYear ? '#1565C0' : '#555', marginBottom: 4 }}>{yr} {isCurrentYear ? '← Current' : ''}</div>
                                    <div style={{ fontSize: 16, fontWeight: 800, color: isCurrentYear ? '#1565C0' : '#333' }}>₹{data.total.toLocaleString('en-IN')}</div>
                                    <div style={{ fontSize: 10, color: '#888', marginTop: 2 }}>Sem I: ₹{data.sem1.toLocaleString('en-IN')}</div>
                                    <div style={{ fontSize: 10, color: '#888' }}>Sem II: ₹{data.sem2.toLocaleString('en-IN')}</div>
                                    {schol > 0 && <div style={{ fontSize: 11, color: '#7B1FA2', fontWeight: 600, marginTop: 4 }}>Net: ₹{Math.max(0, data.total - schol).toLocaleString('en-IN')}</div>}
                                  </div>
                                );
                              })}
                            </div>
